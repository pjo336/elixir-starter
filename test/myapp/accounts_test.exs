defmodule MyApp.AccountsTest do
  ...
  alias MyApp.Accounts.{Organization, User, UserToken}
  ...
  describe "organizations" do
    ...
    test "list_organizations/0 lists all organizations" do
      assert Accounts.list_organizations() == []
      orgz = insert_list(2, :organization)
      assert Accounts.list_organizations() == orgz
    end

    test "get_organization/1 raises an error if not found" do
      assert_raise Ecto.NoResultsError, fn -> Accounts.get_organization!(Ecto.UUID.generate()) end
    end

    test "get_organization!/1 returns the organization by id" do
      orgz = insert_list(2, :organization)
      assert Accounts.get_organization!(List.first(orgz).id) == List.first(orgz)
    end

    test "change_organization/2 returns an organization changeset" do
  ...
  describe "register_user/1" do
    ...
    test "validates email and password when given" do
      {:error, :user, changeset, _} =
        Accounts.register_user(%{email: "not valid", password: "short"})
